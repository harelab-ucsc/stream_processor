...
class SyncNode(Node):
    def __init__(self):
        ...
        self.create_subscription(BuiltinTime, '/pps/time',
                                 self.pps_cb, qos_profile=qos_profile)

        # 2. Camera Streams
        self.create_subscription(Image, '/cam0/camera_node/image_raw',
                                 self.cam0_cb, qos_profile=qos_profile)
        self.create_subscription(Image, '/cam1/camera_node/image_raw',
                                 self.cam1_cb, qos_profile=qos_profile)

        # 3. Navigation & Environment
        self.create_subscription(DIDINS2, '/ins_quat_uvw_lla',
                                 self.ins_cb, qos_profile=qos_profile)
        self.create_subscription(AltSNR, '/rad_altitude',
                                 self.radalt_cb, qos_profile=qos_profile)

        # 4. AS7265x Spectrometer (For Reflectance)
        self.create_subscription(AS7265xCal, '/as7265x/calibrated_values',
    ...
    def csv_read(self):
        self.get_logger().info(
            f'Reading clicks CSV file: {self.clicks_csv}...')
        data = []
        ...
        with open(self.sensors_yaml, 'r') as f:
            ...
            for device in devices:
                ...
                if device == self.sensor_id:
                    ...
                    intr1 = [data["accelerometer_noise_density"],
                             data["accelerometer_random_walk"]]
                    intr2 = [data["gyroscope_noise_density"],
                             data["gyroscope_random_walk"]]
                    self.putParameters(device, res, intr1, intr2, extr)
                elif device == 'radalt':
                    extr = data["T_rad_imu"]
                    self.putParameters(
                        device, res, intr1, intr2,
    ...
    def putParameters(self, device_key, resolution,
                      intrinsics1, intrinsics2, extrinsics):
        vals = '"'
        cols = "sensorID, resolution, intrinsics1, intrinsics2, extrinsics"
        valsList = [device_key, resolution,
                    intrinsics1, intrinsics2, extrinsics]
    ...
    def post_process_and_save(self, data, stamp):
        ...
        try:
            ...
            cam0_raw = self.br.imgmsg_to_cv2(
                data['cam0'], desired_encoding='passthrough').copy()
            cam1_raw = self.br.imgmsg_to_cv2(
                data['cam1'], desired_encoding='passthrough').copy()
